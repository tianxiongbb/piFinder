...
from math import *
from pifinder_basic import fun_read_bed_into_dict
from pifinder_basic import fun_write_list
from pifinder_basic import fun_quick_write
...
def main():
	...
	if len(sys.argv)<2:
		fun_help()
	DICT_PI_CLUSTER=fun_read_bed_into_dict(sys.argv[1])
	...
	for PI_NAME in DICT_PI_CLUSTER.keys():
		...
		if not DICT_SJ.has_key(PI_NAME):
			...
			DICT_SJ_OUT[PI_NAME]=fun_find_intron_for_one_picluster(DICT_PI_CLUSTER[PI_NAME], DICT_SJ[PI_NAME], int(sys.argv[4]))
	LIST_SJ_OUT=[]
	for KEY in DICT_SJ_OUT.keys():
		for i in DICT_SJ_OUT[KEY]:
			LIST_SJ_OUT.append(i[:3]+[KEY]+i[3:])
	fun_write_list(LIST_SJ_OUT,sys.argv[3])
